...
def test_to_et_handles_edt():
    """August = EDT (UTC-4)."""
    utc_dt = datetime(2026, 8, 12, 14, 0, 0, tzinfo=timezone.utc)
    et = to_et(utc_dt)
    assert et.hour == 10
    assert et.minute == 0
...
def test_to_et_handles_est():
    """November = EST (UTC-5)."""
    utc_dt = datetime(2026, 11, 5, 14, 0, 0, tzinfo=timezone.utc)
    et = to_et(utc_dt)
    assert et.hour == 9
...
def test_is_entry_window_excludes_last_15_min():
    ...
    utc_dt = datetime(2026, 8, 11, 19, 50, 0, tzinfo=timezone.utc)  # Tue 15:50 ET
    assert is_market_open(utc_dt) is True
    assert is_entry_window(utc_dt) is False
...
def test_minutes_to_open_friday_evening_to_monday():
    ...
    utc_dt = datetime(2026, 8, 14, 22, 0, 0, tzinfo=timezone.utc)  # Fri 18:00 ET
    m = minutes_to_open(utc_dt)
...
def test_is_weekday_basic():
    ...
    assert is_weekday(datetime(2026, 8, 11, 12, 0, 0, tzinfo=timezone.utc)) is True  # Tue
    assert is_weekday(datetime(2026, 8, 15, 12, 0, 0, tzinfo=timezone.utc)) is False  # Sat
    assert is_weekday(datetime(2026, 8, 16, 12, 0, 0, tzinfo=timezone.utc)) is False  # Sun
...
def test_minutes_to_close_at_open():
    ...
    utc_dt = datetime(2026, 8, 11, 13, 30, 0, tzinfo=timezone.utc)
